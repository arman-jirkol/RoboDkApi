...
namespace SamplePanelRoboDK
{
	public partial class SampleDialog : Form
	{
		...
		private void btnImportGCode_Click(object sender, EventArgs e)
		{
			var gCode = GetGCode();
			if (VerifyGCode(gCode))
				try
				{
					HandleGCode(gCode);
				}
		...
		private void HandleGCode(string gCode)
		{
			var gCodeInterpreter = new GCodeInterpreter(_robot);
			foreach (var commandLine in gCode.Split(new[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries))
			{
				gCodeInterpreter.InterpretLine(commandLine);
			}
		}
